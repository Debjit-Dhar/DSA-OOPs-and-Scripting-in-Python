...
for i in range(len(result)):
    if(i==speak_index[t]):
        result[i]=ecg_signal[beg+speak_index[t]]
...
for i in range(len(non_zero_values)):
    tpeak_index[i]=speak_index[i]
for j in range(len(non_zero_values)-1):
    t=tpeak_index[j]
    for i in range(t,t+101):
        if(ecg_signal[i]>ecg_signal[tpeak_index[j]]):
            tpeak_index[j]=i
for i in range(len(non_zero_values)):
    print(tpeak_index[i],"\t",ecg_signal[tpeak_index[i]])
